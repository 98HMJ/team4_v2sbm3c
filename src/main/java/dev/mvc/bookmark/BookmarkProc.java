...
public class BookmarkProc implements BookmarkProcInter{
  ...
  public ArrayList<BookmarkListVO> list_by_memberno_search_paging(HashMap<String, Object> hashMap) {
     ...
     */
    int begin_of_page = ((int)hashMap.get("now_page") - 1) * Bookmark.RECORD_PER_PAGE;
    ...
    // 2 페이지 = 10 + 10: 20
    // 3 페이지 = 20 + 10: 30
    int end_num = begin_of_page + Bookmark.RECORD_PER_PAGE;
    /*
    ...
    ArrayList<BookmarkListVO> list = this.bookmarkDAO.list_by_memberno_search_paging(hashMap);
  ...
  //  str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성
      str.append("<span class='span_box_1'><A href='" + list_file + "?memberno="+memberno+"&word=" + word + "&now_page=" + _now_page
          + "'>이전</A></span>");
    }
    ...
    for (int i = start_page; i <= end_page; i++) {
      ...
      if (now_page == i) { // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        str.append("<span class='span_box_1'><A href='" + list_file + "?memberno="+memberno+"&word=" + word + "&now_page=" + i + "'>" + i
            + "</A></span>");
      }
    }
  ...
  public int list_by_memberno_search_cnt(HashMap<String, Object> map) {
    int cnt = this.bookmarkDAO.list_by_memberno_search_cnt(map);
    return cnt;
  }
